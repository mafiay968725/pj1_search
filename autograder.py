...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    test_set_1 = ['q1', 'q2', 'q5', 'q6', 'q7']
    test_set_2 = ['q3', 'q4', 'q8', 'q9', 'q10']
    for q in test_subdirs:
        ...
        for t in sorted(tests):
            ...
            if q in test_set_1:
                ...
                testClass = getattr(projectTestClasses2, testDict['class'])
            testCase = testClass(question, testDict)
...
if __name__ == '__main__':
    ...
    testCaseCode = options.testCaseCode.split(',')
    moduleName = re.match('.*?([^/]*)\.py', testCaseCode[0]).group(1)
    moduleDict['projectTestClasses'] = loadModuleFile(moduleName, os.path.join(options.codeRoot, testCaseCode[0]))
    moduleName = re.match('.*?([^/]*)\.py', testCaseCode[1]).group(1)
    moduleDict['projectTestClasses2'] = loadModuleFile(moduleName, os.path.join(options.codeRoot, testCaseCode[1]))
